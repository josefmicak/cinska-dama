    ...
    class VypocetMoznychTahu
    {
        ...
        public (List<MoznyTahLidskehoHrace> mozneTahyLidskehoHrace, List<MoznyTahPocitacovehoHrace> mozneTahyPocitacovehoHrace) pridaniMoznychTahu(int poz_X_KHr, int poz_Y_KHr, int idHrace, int pocetHracu, bool hracSePohlSkokem, int delkaSkokuX, int delkaSkokuY, List<LidskyHrac> poleLidskehoHrace, List<PocitacovyHrac> polePocitacovehoHrace, List<MoznyTahLidskehoHrace> mozneTahyLidskehoHrace, List<MoznyTahPocitacovehoHrace> mozneTahyPocitacovehoHrace)
        {
            ...
            for (int i = 0; i < herniPole.Count; i++)
            {
                ...
                if(idHrace == 3 && (tahDoNejnizsihoANejvyssihoRohu(poz_X_He, poz_Y_He) || tahDoHornihoPravehoRohu(poz_X_He, poz_Y_He) || tahDoSpodnihoLevehoRohu(poz_X_He, poz_Y_He)))
                {
                    continue;
                }

                if(idHrace == 4 && (tahDoNejnizsihoANejvyssihoRohu(poz_X_He, poz_Y_He) || tahDoHornihoLevehoRohu(poz_X_He, poz_Y_He) || tahDoSpodnihoPravehoRohu(poz_X_He, poz_Y_He)))
                {
                    continue;
                }

                if((idHrace == 5 || (idHrace == 1 && pocetHracu == 4)) && (tahDoNejnizsihoANejvyssihoRohu(poz_X_He, poz_Y_He) || tahDoHornihoLevehoRohu(poz_X_He, poz_Y_He) || tahDoSpodnihoPravehoRohu(poz_X_He, poz_Y_He)))
                {
                    continue;
                }

                if((idHrace == 6) && (tahDoNejnizsihoANejvyssihoRohu(poz_X_He, poz_Y_He) || tahDoHornihoPravehoRohu(poz_X_He, poz_Y_He) || tahDoSpodnihoLevehoRohu(poz_X_He, poz_Y_He)))
                {
                    continue;
                }
                ...
                if (Math.Abs(poz_Y_KHr - poz_Y_He) <= posun && (Math.Abs(poz_X_KHr - poz_X_He) <= posun) && !jeTahDuplikatni(poz_X_He, poz_Y_He, poz_X_KHr, poz_Y_KHr, idHrace, mozneTahyLidskehoHrace, mozneTahyPocitacovehoHrace) && !jeHracovoPole(poz_X_He, poz_Y_He, poleLidskehoHrace, polePocitacovehoHrace) && !hracSePohlSkokem)
                {
                    ...
                    {
                        MoznyTahPocitacovehoHrace tah = new MoznyTahPocitacovehoHrace(poz_X_He, poz_Y_He, poz_X_KHr, poz_Y_KHr, sirkaPole, vyskaPole, idHrace);
        ...
        private bool tahDoNejvyssihoRohu(int poz_X_MT, int poz_Y_MT)
        {
            if (poz_Y_MT <= horniOdsazeni + posun * 3)
            {
                return false;
            }
            return true;
        }
        private bool tahDoNejnizsihoRohu(int poz_X_MT, int poz_Y_MT)
        {
            if (poz_Y_MT >= horniOdsazeni + posun * 13)
            {
                return false;
            }
            return true;
        ...
        private bool tahDoNejnizsihoANejvyssihoRohu(int poz_X_MT, int poz_Y_MT)
        {
